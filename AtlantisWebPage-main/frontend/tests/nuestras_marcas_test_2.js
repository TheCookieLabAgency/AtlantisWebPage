...
import "./css/nuestraMarcaTest2.css";
...
const brandPopupInfo = {
  // 2: { info: "Texto adicional", details: ["Item 1", "Item 2"] }
};

const LOGO_DEFAULT_CLASS = "logo-wrapper--small";

function NuestrasMarcasTest2() {
  ...
  const activePopup = activeBrand ? brandPopupInfo[activeBrand.id] : null;
  const hasPopupDetails = Boolean(
    activePopup &&
      (activePopup.info ||
        (activePopup.details && activePopup.details.length > 0))
  );
  const modalLogoSrc = activeBrand ? brandLogos[activeBrand.name] : null;
  const modalLogoClass = activeBrand ? logoSizing[activeBrand.id] || LOGO_DEFAULT_CLASS : LOGO_DEFAULT_CLASS;
  const modalHours = activeBrand?.hours || DEFAULT_HOURS;
  ...
  return (
    <div className="nuestras-marcas">
      ...
      <section className="brands-grid-section">
        <div className="brands-grid">
          {marcasAMostrar.map((marca) => {
            ...
            return (
              <div key={marca.id} className="brand-card" onClick={() => openModal(marca)}>
      ...
      {isModalOpen && activeBrand && (
        <div className="nmt2-modal" onClick={closeModal}>
          <div className="nmt2-modal-panel" onClick={(event) => event.stopPropagation()}>
            <button type="button" className="nmt2-modal-close" onClick={closeModal} aria-label="Cerrar">
              X
            </button>
            <div className="nmt2-modal-content">
              {modalLogoSrc && (
                <div className={`brand-logo-wrapper ${modalLogoClass} nmt2-modal-logo`}>
                  <img src={modalLogoSrc} alt={`Logo ${activeBrand.name}`} className="brand-logo" />
                </div>
              )}
              <div className="nmt2-modal-name">{activeBrand.name}</div>
              {hasPopupDetails && (
                <div className="nmt2-modal-details">
                  {activePopup?.info && <p className="nmt2-modal-text">{activePopup.info}</p>}
                  {activePopup?.details?.length > 0 && (
                    <ul className="nmt2-modal-list">
                      {activePopup.details.map((item, idx) => (
                        <li key={idx}>{item}</li>
                      ))}
                    </ul>
                  )}
                  <div className="nmt2-modal-hours">
                    {modalHours.map((line, idx) => (
                      <span key={idx}>
                        {line}
                        {idx < modalHours.length - 1 && <br />}
                      </span>
                    ))}
                  </div>
                </div>
